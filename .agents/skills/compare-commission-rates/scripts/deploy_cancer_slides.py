...
project_dir = "/Users/garfiwang/Documents/Antigravity-2026/壽險佣金-2026/cancer-commissions"
repo_name = "cancer-commissions"
username = "garfiwang"
...
if success and status.strip():
    success, _ = run_cmd(["git", "commit", "-m", "Initialize Cancer Insurance Commissions Slideshow"])
    if not success:
        sys.exit(1)
else:
...
if success and "origin" in remote_list:
    ...
    success, _ = run_cmd([
        ...
        "--source=.", 
        "--push", 
        "--description", "Cancer Insurance Commissions Presentation"
    ])
    if not success:
