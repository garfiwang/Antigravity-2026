...
markdown_content = []
markdown_content.append("# 三年期壽險商品佣金率列表 (已更新商品與高亮)\n")
markdown_content.append("本列表彙整了資料夾內所有保險公司 PDF 檔案中，繳費年期為 **3年期**（或包含3年的年期範圍，例如 2~4年）的商品。每個公司的表格欄位均與原始檔案完全一致。\n")
markdown_content.append("> [!IMPORTANT]")
markdown_content.append("> 1. 已根據要求**刪除遠雄人壽的 HB4、HZ1、HA4、MC1** 商品。")
markdown_content.append("> 2. **各家保險公司自己旗下首年實際佣金率 (FYC%) 最高之項目已使用 <span style='color:red; font-weight:bold;'>紅字粗體</span> 表示。**")
markdown_content.append("> 3. **初年度服務報酬 (實際首年佣金率 FYC%) = 初年度保費 × 初年度業績換算率 × 40%**\n")

for filename in sorted(files):
    company_name = file_to_company[filename]
    filtered_rows = all_filtered_data[company_name]
    local_max = company_max_fyc[company_name]
    
    markdown_content.append(f"## {company_name} ({filename})\n")
    
    if not filtered_rows:
        markdown_content.append("無符合條件之三年期商品。\n")
        continue
    ...
    for row in filtered_rows:
        ...
        is_local_max = abs(fyc_val - local_max) < 1e-6
        is_global_max = abs(fyc_val - global_max_fyc) < 1e-6
        
        if is_local_max:
            # If it's also global max, maybe we can keep it red bold
            # Actually, both local max and global max are formatted in red bold to follow:
            # "傭金率最高的用紅色字"
...
output_file = "/Users/garfiwang/Documents/Antigravity-2026/壽險佣金-2026/3年期商品佣金率列表.md"
